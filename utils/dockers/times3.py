...
request = ('qa,os="%s",branch="%s",build_type="%s" commit="%s",real=%f,user=%s,sys=%s,mem=%s,inb=%s,oub=%s,maj=%s,nivcsw=%s' % (
    os_name,
    branch,
    build_type,
    commit,
...
requests.post("http://dashboard.neurochaintech.io:8086/write?db=qa", auth=('gitlab', os.environ["INFLUX_PUSH_PASSWORD"]), data=request)
