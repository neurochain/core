...
TEST_F(ForkManager, remove_all) {
  create_first_blocks(10);
  ledger->remove_all();
  ASSERT_EQ(0, ledger->height());
}

TEST_F(ForkManager, get_block) {
...
TEST_F(ForkManager, delete_block) {
...
TEST_F(ForkManager, transactions) {
  messages::Block block0;
  ledger->get_block(0, &block0);
