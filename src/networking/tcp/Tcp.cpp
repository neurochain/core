...
void Tcp::new_connection(std::shared_ptr<bai::tcp::socket> socket,
  ...
  if (!error) {
    ...
    tcp::Connection::Ptr connection_ptr(tcp::Connection::create(
        _current_connection_id, this->id(), _queue, socket, peer, from_remote));

    auto r = _connections.insert(
        std::make_pair(_current_connection_id, connection_ptr));
...
const tcp::Connection::Ptr &Tcp::connection(const Connection::ID id,
                                            bool &found) const {
  std::lock_guard<std::mutex> lock_queue(_connection_mutex);
