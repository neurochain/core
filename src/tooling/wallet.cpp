...
public:
    Wallet(const std::string &pathpriv,
        ...
        messages::config::Database &db)
    {
        ecc_ = std::make_unique<neuro::crypto::Ecc>(pathpriv, pathpub);
        ledger_ = std::make_unique<neuro::ledger::LedgerMongodb>(db);
    ...
    void getTransactions()
    {
        ...
        if (ledger_->for_each(_filter, [&](const neuro::messages::Transaction &a)
    {
        std::string t;
...
int main(int argc, char *argv[])
{
    ...
    Wallet w(keyprivPath, keypubPath, db);
    w.getTransactions();
