...
void PiiConsensus::timer_func() {
    build_block();
    int32_t time_now = std::time(nullptr);
    int32_t next_time = BLOCK_PERIODE - time_now % BLOCK_PERIODE ; // time_now - time_now % BLOCK_PERIODE + BLOCK_PERIODE;
    _timer_of_block_time.expires_at(_timer_of_block_time.expiry() + boost::asio::chrono::seconds(next_time));
    _timer_of_block_time.async_wait(boost::bind(&PiiConsensus::timer_func,this));
    LOG_INFO <<  "Next time in " << std::to_string(next_time);

}
// TO DO Test
void PiiConsensus::build_block() {
    int32_t next_height = next_height_by_time();
    LOG_INFO << "Next height " << std::to_string(next_height);
    if (next_height < 1) {
        return;
    }
    ...
    std::cout << "---" << _wallets_keys.size() << std::endl;
    for(auto &p : _wallets_keys){
        std::cout << humaineaddre(p.first) << std::endl;
    }
    auto it = _wallets_keys.find(next_owner);
    if (it != _wallets_keys.end()) {
        messages::Block blocks;
        _transaction_pool.build_block(blocks, next_height, it->second.get(), 858993459200lu);
